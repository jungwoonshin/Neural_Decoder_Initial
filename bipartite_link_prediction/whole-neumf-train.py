...
def learn_train_adj(seed, model_name):
    ...
    for epoch in range(args.num_epoch):
        ...
        # train_edges_part = np.concatenate((train_edges_part_half, reverse_edge(train_edges_part_half))) 
        # train_false_edges_part_half = train_false_edges[:int(args.subsample_number/2)] 
        # train_false_edges_part = np.concatenate((train_false_edges_part_half,  reverse_edge(train_false_edges_part_half)))
        ...
        if (epoch + 1) % 20 == 0:
            ...
            test_roc, test_ap = get_scores(test_edges, test_edges_false, A_pred.cpu(), adj_orig)
            epoch_performance[(epoch+1)].append(test_roc)
...
def run():
    ...
    epoch_performance = defaultdict(list)
    ...
    for seed in range(args.numexp):
        ...
        if args.subsample_number > len(train_edges):
            args.subsample_number = 2 * len(train_edges)
        ...
        adj_train_norm, test_roc_pretrain, test_ap_pretrain = learn_train_adj(seed, 'WHOLE_ND_LGAE')
    ...
    epoch_permance_list = []
    for key,val in epoch_performance.items():
        # print(key + ':' + np.mean(val))
        epoch_permance_list.append((key, np.mean(val)))

    sorted_by_second = sorted(epoch_permance_list, key=lambda tup: tup[1],reverse=True)
